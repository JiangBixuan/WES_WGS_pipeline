...
mkdir Annotation
inputtarget=/home/zhouwei/zhou_data/WES/VariantFiltering/metsExon100.Filtered_SNP.vcf
outpath4step=/home/zhouwei/zhou_data/WES/VariantFiltering/Annotation
outtarget=(${outpath4step}/metsExon100_SNP.annovar.txt)
###software
...
if [ ${sample} == multiple ] ; then
	...
	para2=metsExon100_SNP.avinput
else
	para1="--format vcf4 --allsample"
	para2=annovar.metsExon100_SNP.avinput
fi

perl ${annovar}/convert2annovar.pl --includeinfo ${para1} \
	--outfile ${outpath4step}/metsExon100_SNP.avinput \
	${inputtarget}
...
perl ${annovar}/table_annovar.pl --buildver hg19 \
	...
	--outfile ${outpath4step}/metsExon100_SNP.annovar
	-csvout > ${outpath4step}/metsExon100_SNP.annovar.log
